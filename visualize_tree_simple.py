    ...
    for i, (path, rule, match) in enumerate(paths):
        ...
        for node_id in node_ids:
            ...
            if node_str in modified_dot_data:
                ...
                # 添加边框颜色和宽度
                new_attrs = node_attrs + f', color="{color}", penwidth=3.0'
                # 替换原来的属性
        ...
        for j in range(len(node_ids) - 1):
            ...
            if edge_str in modified_dot_data:
                ...
                if '[' in modified_dot_data[start_idx:end_idx]:
                    ...
                    # 添加颜色属性
                    new_attrs = modified_dot_data[attr_start + 1:attr_end] + f', color="{color}", penwidth=2.0'
                    # 替换原来的属性
                    modified_dot_data = modified_dot_data[:attr_start + 1] + new_attrs + modified_dot_data[attr_end:]
                else:
                    # 如果没有属性，添加属性
                    new_edge = f'{edge_str} [color="{color}", penwidth=2.0]'
                    modified_dot_data = modified_dot_data[:start_idx] + new_edge + modified_dot_data[end_idx:]
        
        # 添加图例信息到DOT数据中
        path_desc = f"路径 {i+1}: {rule}"
        legend_info.append((color, path_desc))
    
    # 在DOT数据中添加图例
    if legend_info:
        legend_dot = "\n  subgraph cluster_legend {\n    label=\"路径图例\";\n    fontsize=14;\n    style=filled;\n    color=lightgrey;\n"
        for i, (color, desc) in enumerate(legend_info):
            legend_dot += f'    legend_{i} [shape=box, style=filled, fillcolor="{color}", label="{desc}"];\n'
        legend_dot += "  }\n"
        
        # 在DOT数据的末尾插入图例
        end_idx = modified_dot_data.rfind('}')
        modified_dot_data = modified_dot_data[:end_idx] + legend_dot + modified_dot_data[end_idx:]
...
def main():
    ...
    print("\n可视化完成，请查看输出目录中的图像文件。")
    print(f"决策树图像: {os.path.join(output_dir, 'decision_tree.png')}")
    print(f"决策路径图像: {os.path.join(output_dir, 'decision_paths.png')}")
